...
(navy headings, styled code/tables, callout blockquotes). Adapted from
Boord's script of the same name - same look and feel, on purpose.
...
def ensure_blank_before_lists(text):
    ...
    KNOWN TRAP: this cannot tell a real list item from a *wrapped sentence*
    whose next line happens to begin with "- ", which is easy to write when
    a dash is being used as punctuation. Write the markdown so a "- " never
    starts a line unless it really is a bullet. To check a rendered file:

        pdftotext MANUAL.pdf - | grep '\\*\\*'

    Any hit means a bold span got split across a paragraph break; a
    correctly rendered PDF contains no literal **.
    """
    lines = text.split('\n')
    out = []
    in_fence = False
...
def main():
    ...
    # own page. Shorter docs shouldn't force a page per section - pass
    # "compact" as a 3rd arg to let content flow naturally.
