...
"@
    ...
    Write-Host " tailnet. Once this PC is connected, run this ONCE on this PC to"
    Write-Host " publish Kudde's port over Tailscale HTTPS:"
    Write-Host ""
    Write-Host "   tailscale serve --bg --https=$Port http://localhost:$Port"
    Write-Host ""
    Write-Host " (If another app on this PC already answers on plain https://.ts.net/"
    Write-Host " with no port - Boord uses that slot - do NOT re-run its bare form for"
    Write-Host " Kudde: that silently steals the slot and breaks the other app instead"
    Write-Host " of erroring. The --https=$Port form above is Kudde's own slot and"
    Write-Host " cannot collide with it. Run 'tailscale serve status' to see what is"
    Write-Host " already claimed.)"
    Write-Host ""
    Write-Host " Then run 'tailscale status' here to find this PC's name, and open"
    Write-Host " https://<name>.<tailnet>.ts.net:$Port/field/ or :$Port/admin/ from any"
    Write-Host " connected device - including this one; localhost does not get an"
    Write-Host " exemption."
    Write-Host ""
